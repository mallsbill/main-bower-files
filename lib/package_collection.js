...
PackageCollection.prototype = {
    ...
    collectPackages: function() {
        ...
        try {
            this.bowerJson = JSON.parse(stripJsonComments(readFile(this.opts.paths.bowerJson, 'utf8')));
            var devDependencies = this.bowerJson.devDependencies || {},
                dependencies = this.bowerJson.dependencies || {},
                main = this.bowerJson.main || {};
        }  catch(err) {
        ...
        this.overrides = extend(this.bowerJson.overrides || {}, this.overrides);

        this.checkGroupExists(group, this.bowerJson, function (missingGroup) {
            throw new Error('group "' + missingGroup + '" does not exists in bower.json');
        });

        if (includeDev !== 'exclusive') {
            this.addDependencies(dependencies, group, this.bowerJson);
        }

        if (includeDev !== false) {
            this.addDependencies(devDependencies, group, this.bowerJson);
        }

        if (includeSelf !== false) {
            this.add(this.bowerJson.name || 'self', path.dirname(this.opts.paths.bowerJson), main);
        }
    },
    ...
    /**
     * Order package by order of specified in the group
     */
    orderQueue: function() {
        if (this.order === "group" && this.opts.group !== undefined) {
            let bowerJson = this.bowerJson;
            let bowerJsonGroup = [];

            if (
                typeof this.opts.group === "string" &&
                this.opts.group.charAt(0) !== "!"
            ) {
                bowerJsonGroup = bowerJson.group[this.opts.group];
            } else if (typeof this.opts.group === "object") {
                this.opts.group.forEach(function(group) {
                    if (group.charAt(0) !== "!") {
                        bowerJsonGroup = bowerJsonGroup.concat(
                            bowerJson.group[group]
                        );
                    }
                });
            } else {
            ...
            this._queue.sort(function(packageA, packageB) {
                return (
                    bowerJsonGroup.indexOf(packageA.name) -
                    bowerJsonGroup.indexOf(packageB.name)
                );
            });
        }
    },
